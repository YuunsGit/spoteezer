  ...
  return (
    <>
      <section className="pb-4">
        <div className="h-12 w-1/3 animate-pulse rounded-full bg-gray-400" />
        <div className="mt-5 grid grid-cols-1 gap-x-6 gap-y-3 @md:grid-cols-2 @4xl:grid-cols-3 @4xl:gap-y-4">
          {Array(6)
            ...
            .map((_, i) => (
              <div
                key={i}
                className="group relative flex h-16 overflow-hidden rounded bg-[hsla(0,0%,100%,.1)] transition-colors duration-300 @4xl:h-20"
              >
                <div className="size-20 animate-pulse bg-gray-400 shadow-[0_8px_24px_rgb(0_0_0_/_50%)]" />
                <div className="flex w-full items-center justify-between px-4">
                  <div className="h-7 w-full animate-pulse rounded-full bg-gray-400" />
                </div>
              </div>
            ))}
        </div>
